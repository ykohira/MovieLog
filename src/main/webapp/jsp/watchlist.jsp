...
<%@ page import="java.util.*, dto.WatchlistDTO"%>
<html>
<head>
<title>ウォッチリスト</title>
...
<body>
	...
	<h1 style="text-align: center; margin-top: 40px;">ウォッチリスト</h1>
	...
	<%
	List<WatchlistDTO> watchlist = (List<WatchlistDTO>) request.getAttribute("watchlist");
	if (watchlist != null && !watchlist.isEmpty()) {
	%>
	<div class="movie-grid">
		<%
		for (WatchlistDTO movie : watchlist) {
		%>
		<!-- 各映画のジャンルを data-genre に設定 -->
		<div class="movie-card" data-genre="<%=movie.getGenre()%>">
			<a href="/movieLog/watchlistDetail?id=<%=movie.getId()%>"
